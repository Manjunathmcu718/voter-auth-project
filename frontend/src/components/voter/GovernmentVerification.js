...
const GovernmentVerification = ({ voterData, onVerificationComplete }) => {
    ...
    // Automatically start verification when the component loads
    useEffect(() => {
        const startVerification = async () => {
            setIsVerifying(true);
            setError('');
            try {
                const response = await axios.post('http://localhost:5000/api/auth/verify-government-id', voterData);
                setVerificationResults(response.data);
                setVerificationStatus(response.data.overall_status.status);
                onVerificationComplete(response.data);
            } catch (err) {
                const errorMsg = err.response?.data?.error || 'An unknown error occurred during verification.';
                setError(`Verification Failed: ${errorMsg}`);
                setVerificationStatus('ERROR');
                onVerificationComplete(null); // Signal that verification failed
            }
    ...
    }, [voterData, onVerificationComplete]);

    const getStatusIcon = (status) => {
        switch (status) {
            case 'VERIFIED': return <CheckCircle className="w-5 h-5 text-green-600" />;
            case 'SUSPICIOUS':
            case 'FLAGGED': return <AlertTriangle className="w-5 h-5 text-yellow-600" />;
            case 'FORGED':
    ...
    const renderVerificationDetails = () => {
        ...
        // If results haven't loaded, or details are missing, show a loading message.
        if (!verificationResults || !verificationResults.details) {
            return <p className="text-center text-gray-600">Loading verification details...</p>;
        }

        const { details, overall_status } = verificationResults;

        return (
            <div className="space-y-4">
                <div className={`p-4 rounded-lg border-l-4 ${
                    overall_status.status === 'VERIFIED' ? 'bg-green-50 border-green-500' : 'bg-red-50 border-red-500'
                }`}>
                    <h3 className="font-bold text-lg flex items-center gap-2">
                        {getStatusIcon(overall_status.status)}
                        Overall Status: {overall_status.status.replace(/_/g, ' ')}
                    </h3>
                    <p className="text-sm text-gray-700 mt-1">{overall_status.message}</p>
                </div>
                
                {Object.entries(details).map(([key, value]) => (
                    <div key={key} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex justify-between items-center">
                            <h4 className="font-semibold text-gray-800">{key.replace(/_/g, ' ').toUpperCase()}</h4>
                ...
                <div className="mt-6">
                    {isVerifying && (
                        <div className="text-center space-y-3">
                            ...
                            <p className="font-semibold text-gray-700">Verifying with Government Databases...</p>
                    ...
                    {verificationResults && !isVerifying && renderVerificationDetails()}
                </div>
            </div>
        </motion.div>
    );
